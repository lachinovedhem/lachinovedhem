...
| **[form-recalc-graph](https://github.com/lachinovedhem/form-recalc-graph)** | A form's reactivity as **data instead of onChange handlers** — the dependency graph you never drew, stored as rows and evaluated in topological order, so a new derived field ships as an `INSERT`. Cycles are named, not hung on; late lookups are discarded rather than overwriting newer input. Includes a zero-dependency playground that draws the graph and lights the edges in recompute order. |
| **[interactive-transactions](https://github.com/lachinovedhem/interactive-transactions)** | An operation that can **ask the operator a question** mid-flight — *"3 open orders reference this customer, delete anyway?"* — without holding a database transaction across human think-time. The driver rolls back before it asks, hands back the evidence rows and a signed continuation, and re-runs when the answer arrives. Bounded rounds, idempotent replay. |
| **[rule-table-pricing](https://github.com/lachinovedhem/rule-table-pricing)** | Nine kinds of retail promotion, **one evaluator, zero deployments** — a discount is a row in a table. One primitive lowers a price so no rule can bypass the floor; a free unit is valued at current net, not list; usage caps are *counted* from an append-only ledger because an increment is lost or doubled when two offline tills sync. The receipt explains every cent, including why a rule did **not** fire. |
